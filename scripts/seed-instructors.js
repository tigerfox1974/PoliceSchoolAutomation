// 66. Dönem Polis ve İtfaiye Temel Eğitimi Eğitmen Kadrosu
// KKTC Kimlik: 10 haneli (test verileri)
...
async function seedInstructors() {
  console.log('🚀 Eğitmenler ekleniyor...\n');
  ...
  for (const instructor of instructors) {
    try {
      ...
      if (response.ok) {
        console.log(`✅ ${instructor.rank} ${instructor.firstName} ${instructor.lastName}`);
        success++;
      } else {
        console.log(`❌ ${instructor.firstName} ${instructor.lastName} - HATA: ${result.error}`);
        failed++;
      }
    } catch (error) {
      console.log(`❌ ${instructor.firstName} ${instructor.lastName} - Bağlantı hatası`);
      failed++;
    }
  }
  ...
  console.log(`\n📌 KKTC Kimlik: 10 haneli format kullanıldı`);
}
